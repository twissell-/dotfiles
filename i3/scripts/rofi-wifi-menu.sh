...
option="$(echo -e "${options}" | rofi -dmenu -p "Wi-Fi menu: ")" # -lines "$LINENUM" -a "$HIGHLINE" -location "$POSITION" -yoffset "$YOFF" -xoffset "$XOFF" -font "$FONT" -width -"$RWIDTH"
...
case "${option}" in
    ...
    "Manual connection")
        # Manual entry of the SSID and password (if appplicable)
        input=$(echo "enter the SSID of the network (SSID,password)" | rofi -dmenu -p "Manual Entry: " -font "$FONT" -lines 1)
        # Separating the password from the entered string
